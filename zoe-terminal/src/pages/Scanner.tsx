  ...
  return (
    <div className="space-y-6 sm:space-y-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2 border-b border-border pb-4 sm:pb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl font-semibold text-white tracking-tighter">Market Scanner</h2>
            <p className="text-xs sm:text-sm text-text-muted mt-1 font-medium tracking-tight">
              {candidates.length} symbols scored
            </p>
          </div>
          <div className="bg-surface-highlight/50 px-3 py-1.5 rounded-xl border border-border text-[9px] sm:text-[10px] font-black text-white uppercase tracking-[0.15em]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
          {candidates.length > 0 ? candidates.map(candidate => {
              ...
              return (
              <div key={candidate.id} className="card-premium p-4 sm:p-8 group overflow-hidden relative">
