...
export default function Intelligence() {
  ...
  const [planSource, setPlanSource] = useState<'gameplan' | 'scanner'>('gameplan');
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        ...
        if (plan) {
          setActiveTab((plan as any).status);
          setPlanSource('gameplan');
    ...
    return candidates
      ...
      .map((c) => {
        ...
        return {
          ...
          ivr: info?.ivr ?? '—',
          trend: breakdown?.trend ?? 0, value: breakdown?.value ?? 0, volatility: breakdown?.volatility ?? 0,
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex justify-between items-end border-b border-border pb-8">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tighter">Intelligence Center</h2>
      ...
      <div className="flex gap-6 border-b border-border/50">
        {SECTIONS.map((sec) => (
          ...
          >
            <sec.icon className={cn("w-3.5 h-3.5", activeSection === sec.id ? "text-profit" : "")} />
            {sec.label}
          </button>
        ))}
      </div>
      ...
      {activeSection === 'plan' && (
        <div className="space-y-8">
          ...
          <div className="flex gap-8 border-b border-border/50">
            {[
              ...
              <button
                ...
                className={cn(
                  "pb-3 text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 border-b-2 transition-all",
                  activeTab === tab.id ? "border-profit text-white" : "border-transparent text-text-muted hover:text-text-secondary"
                )}
              >
                <tab.icon className={cn("w-3 h-3", activeTab === tab.id ? "text-profit" : "")} /> {tab.label}
              </button>
            ))}
          </div>
          ...
          {planLoading || dashLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="card-premium p-6">
                  <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-text-muted mb-5">Current Holdings</h3>
            ...
            <div className="text-text-muted text-xs italic py-8 text-center">Curated gameplan loaded — view in table mode.</div>
          ) : (
            <EmptyPanel icon={Lock} text="No Intelligence Record Found" sub="Zoe has not finalized the session strategy yet." />
          )}
        </div>
      )}
      ...
      {activeSection === 'structure' && (
        <div className="space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <FilterBar label="Timeframe" items={TIMEFRAMES} active={structTimeframe} onSelect={setStructTimeframe} />
          </div>
                ...
                <div className="card-premium p-8">
                  ...
                  <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                    {trendlines.length > 0 ? trendlines.map((tl) => (
                      <div key={tl.id} className="grid grid-cols-[1fr_auto_auto_auto] items-center gap-3 bg-background/50 border border-border rounded-lg px-4 py-3 text-xs">
                ...
                <div className="card-premium p-8">
                  ...
                  <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                    {levels.length > 0 ? levels.map((lv) => (
                      <div key={lv.id} className="grid grid-cols-[1fr_auto_auto_auto_auto] items-center gap-3 bg-background/50 border border-border rounded-lg px-4 py-3 text-xs">
                        <div className="flex items-center gap-2">
                ...
                <div className="card-premium p-8">
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                    {structureEvents.length > 0 ? structureEvents.map((ev) => (
                      <div key={ev.id} className="flex items-center gap-3 bg-background/50 border border-border rounded-lg px-4 py-3">
                ...
                <div className="card-premium p-8">
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                    {bounceEvents.length > 0 ? bounceEvents.map((be) => (
                      <div key={be.id} className="flex items-center gap-3 bg-background/50 border border-border rounded-lg px-4 py-3">
...
function KPIBlock({ icon: Icon, label, value, color }: { icon: typeof DollarSign; label: string; value: string; color: string }) {
  ...
  return (
    <div className="card-premium p-5 flex items-center gap-4">
      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", bgMap[color] ?? 'bg-white/5')}>
        <Icon className={cn("w-5 h-5", colorMap[color] ?? 'text-white/60')} />
      </div>
      <div>
        <div className="text-[10px] font-black text-text-muted uppercase tracking-[0.15em]">{label}</div>
        <div className={cn("text-lg font-black tracking-tight tabular-nums", colorMap[color] ?? 'text-white')}>{value}</div>
      </div>
    </div>
  );
}
