  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-end border-b border-border pb-8">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tighter">Chart Analysis</h2>
          <p className="text-sm text-text-muted mt-2 font-medium tracking-tight">
            Candlestick patterns, multi-timeframe trends &amp; support/resistance
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        {analysis.mtfAlignment !== null && (
          <div className={cn(
            'flex items-center gap-2 px-4 py-1.5 rounded-lg border text-xs font-bold',
      ...
      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-border bg-surface-highlight/20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {symbolPrices[selectedSymbol] && (
              <span className="text-sm font-mono text-white">${symbolPrices[selectedSymbol].toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            )}
          </div>
        ...
        <div className="p-2">
          {loading ? (
            <div className="flex items-center justify-center h-[400px] text-text-muted animate-pulse">
              Loading chart data...
            </div>
          ) : candles.length > 0 ? (
            <CandlestickChart
            ...
            <div className="flex flex-col items-center justify-center h-[400px] text-text-muted gap-3">
        ...
        <div className="bg-surface border border-border rounded-xl p-6">
          ...
          <div className="flex flex-wrap gap-3">
            {analysis.supportLevels.map((l, i) => (
              <span key={`s-${i}`} className="px-3 py-1.5 bg-profit/10 text-profit border border-profit/20 rounded-lg text-xs font-mono font-bold">
            ...
            {analysis.resistanceLevels.map((l, i) => (
              <span key={`r-${i}`} className="px-3 py-1.5 bg-loss/10 text-loss border border-loss/20 rounded-lg text-xs font-mono font-bold">
